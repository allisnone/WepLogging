...
def get_html(url='',refer_url=''):
    #url = 'http://kaijiang.500.com/shtml/ssq/19018.shtml'
...
def my_company_abormal(url,keyword='异常'):
    ...
    address = re.findall(r'<span id="OperationSite">(.*?)<br />.*</span>',html,re.S|re.M) #广州市天河区广棠路23号A栋（部位：一楼115）
    address = address[0].replace('\r','').replace('\n','')
    if address=='广州市天河区广棠路23号A栋（部位：一楼115）':
        print('注册地址：', address)
    else:
        print('新地址变更为：', address)
